...
class TimelineGateLatticeTranslationKernel:
    ...
    def _quantize_signature(self, vector: Dict[str, Any]) -> Dict[str, int]:
        ...
        phase_bin = min(
            self.phase_bins - 1,
            max(0, int(vector["phase"] * self.phase_bins)),
        )
        causal_bin = min(
            self.causal_bins - 1,
            max(0, int(vector["causal_score"] * self.causal_bins)),
        )
    ...
    def _timeline_axis(self, timeline_id: str) -> int:
        ...
        # Stable hash instead of Python's randomized hash()
        h = hashlib.sha1(timeline_id.encode("utf-8")).hexdigest()
    ...
    def _build_global_vector_trace(
        ...
        avg_gate = sum(
            1.0 if s["vector"]["gate_mask"] else 0.0
            for s in sigs
        ) / len(sigs)
    ...
    def _build_temporal_edges(
        ...
        for tl in timelines:
            ...
            for a, b in zip(ordered, ordered[1:]):
                if a.id not in event_signatures or b.id not in event_signatures:
                    continue
    ...
    def _build_gate_edges(
        ...
        for gate in gates:
            if (
                gate.src_event_id not in event_signatures
                or gate.dst_event_id not in event_signatures
            ):
                edges.append({
                    "src": gate.src_event_id,
                    "dst": gate.dst_event_id,
                    "type": f"gate_missing:{gate.id}",
                    "weight": 0.0,
                    "missing": True,
                })
                continue
...
# =========================================================
# Convenience API for the larger Le'Veon runtime
# =========================================================

def translate_timeline_gates(
    timelines: List[Timeline],
    gates: List[Gate],
    *,
    question: str = "How do timelines and time gates map into lattice structure?",
    ...
    phase_bins: int = 10,
    causal_bins: int = 10,
    causal_edge_threshold: float = 0.95,
) -> TimelineTranslationResult:
    kernel = TimelineGateLatticeTranslationKernel(
        time_bin_size=time_bin_size,
        phase_bins=phase_bins,
        causal_bins=causal_bins,
        causal_edge_threshold=causal_edge_threshold,
    )
...
def demo() -> TimelineTranslationResult:
    t0 = Timeline(
        id="T0",
        events=[
    ...
    return translate_timeline_gates(
        [t0, t1],
        gates,
