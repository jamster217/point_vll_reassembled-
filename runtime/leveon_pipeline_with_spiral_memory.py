...
# --- LIVE MASTER API OUTPUT OVERRIDE ---
# Repairs stale compound-memory replies in chatroom without replacing the whole pipeline.
try:
    _leveon_pipeline_before_live_master_api = leveon_pipeline

    def leveon_pipeline(text, *args, **kwargs):
        out = _leveon_pipeline_before_live_master_api(text, *args, **kwargs)
        try:
            from runtime.live_master_api_response import apply_live_master_api

            if isinstance(out, dict):
                meta = out.setdefault("meta", {})

                meta["pre_live_master_renderer"] = str(
                    meta.get("renderer_source", "")
                )

                meta["pre_live_master_has_answer"] = bool(
                    out.get("answer") or out.get("final_english")
                )

            result = apply_live_master_api(str(text or ""), out)

            if isinstance(result, dict):
                rmeta = result.setdefault("meta", {})

                rmeta["post_live_master_renderer"] = str(
                    rmeta.get("renderer_source", "")
                )

                rmeta["post_live_master_has_answer"] = bool(
                    result.get("answer") or result.get("final_english")
                )

            return result
        except Exception as e:
            if isinstance(out, dict):
                out.setdefault("meta", {})["live_master_api_error"] = str(e)
            return out
except Exception:
    pass
